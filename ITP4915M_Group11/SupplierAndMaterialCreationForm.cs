...
namespace ITP4915M_Group11
{
    ...
    public partial class SupplierAndMaterialCreationForm : Form
    {
        ...
        private void SetupPremiumUI()
        {
            ...
            rbExistingSupplier.CheckedChanged += SupplierMode_CheckedChanged; rbNewSupplier.CheckedChanged += SupplierMode_CheckedChanged;
            ...
            rbNewMaterial.CheckedChanged += MaterialMode_CheckedChanged; rbExistingMaterial.CheckedChanged += MaterialMode_CheckedChanged;
            ...
            // ==========================================
            // 右邊：Live Monitor DataGridView & Search
            // ==========================================
            ...
            dgvSuppliers = new DataGridView { Location = new Point(15, 45), Size = new Size(560, 455), BackgroundColor = Color.White, BorderStyle = BorderStyle.None, AllowUserToAddRows = false, AllowUserToDeleteRows = false, ReadOnly = true, SelectionMode = DataGridViewSelectionMode.FullRowSelect, MultiSelect = false, AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill, AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells, RowHeadersVisible = false, GridColor = Color.FromArgb(226, 232, 240), EnableHeadersVisualStyles = false };
        ...
        private void TxtSearch_TextChanged(object sender, EventArgs e)
        {
            if (dgvSuppliers.DataSource is DataTable dt)
            {
                ...
                {
                    // 可以同時 Search ID, Supplier 名, Contact 同 Material
                    dt.DefaultView.RowFilter = $"[ID] LIKE '%{keyword}%' OR [Supplier Name] LIKE '%{keyword}%' OR [Contact Reference] LIKE '%{keyword}%' OR [Provided Materials] LIKE '%{keyword}%'";
                }
            }
        }
        ...
        private void RefreshSupplierGrid()
        {
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                ...
                {
                    conn.Open();
                    string query = @"SELECT s.SupplierID AS `ID`,s.SupplierName AS `Supplier Name`,IF(s.IsActive=1,'Active','🔴 Deactivated') AS `Status`,s.ContactInfo AS `Contact Reference`,IFNULL(GROUP_CONCAT(CONCAT(rm.MaterialName,' (',rm.MaterialID,')') SEPARATOR ',\n'),'(No Materials Linked)') AS `Provided Materials` FROM supplier s LEFT JOIN supplier_material sm ON s.SupplierID=sm.SupplierID LEFT JOIN raw_material rm ON sm.MaterialID=rm.MaterialID GROUP BY s.SupplierID ORDER BY s.IsActive DESC,s.SupplierID DESC";
                    using (MySqlDataAdapter adapter = new MySqlDataAdapter(query, conn))
                    {
                        DataTable dt = new DataTable(); adapter.Fill(dt);
        ...
        private void btnUpdateSupplier_Click(object sender, EventArgs e)
        {
            if (editingSupplierID == null)
            {
                ...
                rbExistingSupplier.Enabled = false; rbNewSupplier.Enabled = false;
                pnlMaterial_SetEnabled(false);
                txtNewSuppName.Text = oldName; txtNewSuppContact.Text = oldContact;
                txtNewSuppName.ReadOnly = false; txtNewSuppContact.ReadOnly = false;
                txtNewSuppName.BackColor = Color.White; txtNewSuppContact.BackColor = Color.White;
                btnUpdateSupplier.Text = "💾 Save Supplier Changes"; btnUpdateSupplier.BackColor = Color.FromArgb(37, 99, 235);
                btnDeleteSupplier.Text = "❌ Cancel Editing"; btnDeleteSupplier.BackColor = Color.FromArgb(100, 116, 139);
                ...
                if (confirm == DialogResult.Yes)
                {
                    using (MySqlConnection conn = new MySqlConnection(connString))
                    {
                        try
                        {
                            ...
                            {
                                cmd.Parameters.AddWithValue("@name", updatedName); cmd.Parameters.AddWithValue("@contact", updatedContact); cmd.Parameters.AddWithValue("@id", editingSupplierID);
                                cmd.ExecuteNonQuery();
                            }
        ...
        {
            rbNewMaterial.Enabled = enabled; rbExistingMaterial.Enabled = enabled; cmbExistingMaterials.Enabled = enabled;
            txtMatName.Enabled = enabled; txtMatPrice.Enabled = enabled; txtMatReorderLevel.Enabled = enabled;
        ...
        private void btnDeleteSupplier_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show($"Are you sure you want to DEACTIVATE supplier:\n\n[{selectedSuppName}] (ID: {selectedSuppID})?\n\nHistorical records will be preserved for auditing, but it will be hidden from new procurement dropdowns.", "Supplier Status Change", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    try
                    {
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(updateQuery, conn)) { cmd.Parameters.AddWithValue("@id", selectedSuppID); cmd.ExecuteNonQuery(); }
                        MessageBox.Show($"Supplier [{selectedSuppName}] has been deactivated successfully.");
                        LoadExistingSuppliers(); RefreshSupplierGrid();
                    }
                    catch (Exception ex) { MessageBox.Show("Error updating supplier status:\n" + ex.Message); }
                }
            }
        }
        ...
        private void ResetFormEditContext()
        {
            ...
            txtNewSuppName.Clear(); txtNewSuppContact.Clear();
            rbExistingSupplier.Enabled = true; rbNewSupplier.Enabled = true; pnlMaterial_SetEnabled(true);
            rbExistingSupplier.Checked = true;
            btnUpdateSupplier.Text = "✏️ Update Selected Supplier"; btnUpdateSupplier.BackColor = Color.FromArgb(245, 158, 11);
            btnDeleteSupplier.Text = "🗑️ Deactivate Selected Supplier"; btnDeleteSupplier.BackColor = Color.FromArgb(239, 68, 68);
            LoadExistingSuppliers(); RefreshSupplierGrid();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string matNameStr = txtMatName.Text.Trim(); string priceStr = txtMatPrice.Text.Trim(); string reorderStr = txtMatReorderLevel.Text.Trim();
            ...
            string finalSuppID = ""; string finalMatID = "";
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                        ...
                        try
                        {
                            if (!isUnassigned)
                            {
                                ...
                                else
                                {
                                    ...
                                    using (MySqlCommand cmdMaxSupp = new MySqlCommand(getSuppIdQuery, conn, trans)) { object result = cmdMaxSupp.ExecuteScalar(); if (result != null && result != DBNull.Value) { string maxId = result.ToString(); if (maxId.Length == 4 && int.TryParse(maxId.Substring(1), out int currentInt)) finalSuppID = "V" + (currentInt + 1).ToString("D3"); else finalSuppID = "V001"; } else finalSuppID = "V001"; }
                                    string insertSuppSql = "INSERT INTO supplier (SupplierID,SupplierName,ContactInfo) VALUES (@id,@name,@contact)";
                                    using (MySqlCommand cmdInsSupp = new MySqlCommand(insertSuppSql, conn, trans)) { cmdInsSupp.Parameters.AddWithValue("@id", finalSuppID); cmdInsSupp.Parameters.AddWithValue("@name", finalSuppName); cmdInsSupp.Parameters.AddWithValue("@contact", txtNewSuppContact.Text.Trim()); cmdInsSupp.ExecuteNonQuery(); }
                                }
                            }
                            if (rbExistingMaterial.Checked)
                            {
                                ...
                                using (MySqlCommand cmdCheck = new MySqlCommand(checkLinkSql, conn, trans)) { cmdCheck.Parameters.AddWithValue("@s", finalSuppID); cmdCheck.Parameters.AddWithValue("@m", finalMatID); if (Convert.ToInt32(cmdCheck.ExecuteScalar()) > 0) throw new Exception($"Material [{finalMatID}] is already linked to Supplier [{finalSuppID}]."); }
                            }
                            else
                            {
                                string checkNameSql = "SELECT MaterialID FROM raw_material WHERE LOWER(MaterialName)=LOWER(@name) LIMIT 1";
                                using (MySqlCommand cmdCheckName = new MySqlCommand(checkNameSql, conn, trans)) { cmdCheckName.Parameters.AddWithValue("@name", matNameStr); object res = cmdCheckName.ExecuteScalar(); if (res != null) throw new Exception($"Material Name '{matNameStr}' already exists (ID: {res.ToString()}).\n\nPlease use 'Link Existing Material' instead."); }
                                string getMatIdQuery = "SELECT MaterialID FROM raw_material WHERE MaterialID LIKE 'RM___' ORDER BY MaterialID DESC LIMIT 1";
                                using (MySqlCommand cmdMaxMat = new MySqlCommand(getMatIdQuery, conn, trans)) { object matResult = cmdMaxMat.ExecuteScalar(); if (matResult != null && matResult != DBNull.Value) { string maxMatId = matResult.ToString(); if (maxMatId.Length == 5 && int.TryParse(maxMatId.Substring(2), out int currentMatInt)) finalMatID = "RM" + (currentMatInt + 1).ToString("D3"); else finalMatID = "RM001"; } else finalMatID = "RM001"; }
                                decimal finalPrice = decimal.Parse(priceStr); int finalReorder = string.IsNullOrWhiteSpace(reorderStr) ? 20 : int.Parse(reorderStr);
                                ...
                                using (MySqlCommand cmdInsMat = new MySqlCommand(insertMatSql, conn, trans)) { cmdInsMat.Parameters.AddWithValue("@id", finalMatID); cmdInsMat.Parameters.AddWithValue("@name", matNameStr); cmdInsMat.Parameters.AddWithValue("@cost", finalPrice); cmdInsMat.Parameters.AddWithValue("@reorder", finalReorder); cmdInsMat.ExecuteNonQuery(); }
                            }
                            if (!isUnassigned)
                            {
                                string insertLinkSql = "INSERT INTO supplier_material (SupplierID,MaterialID) VALUES (@suppID,@matID)";
                                using (MySqlCommand cmdLink = new MySqlCommand(insertLinkSql, conn, trans)) { cmdLink.Parameters.AddWithValue("@suppID", finalSuppID); cmdLink.Parameters.AddWithValue("@matID", finalMatID); cmdLink.ExecuteNonQuery(); }
                            }
                            trans.Commit();
                            if (rbExistingMaterial.Checked) MessageBox.Show($"Success! Existing Material [{finalMatID}] has been linked to Supplier [{finalSuppID}].", "Link Successful", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            else if (isUnassigned) MessageBox.Show($"Material (ID: {finalMatID}) created successfully!\n\n⚠️ Note: No supplier is linked yet.", "Created Without Supplier", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            else if (rbNewSupplier.Checked) MessageBox.Show($"Success! New Supplier [{finalSuppName}] and Material [{finalMatID}] registered.", "Transaction Successful", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            else MessageBox.Show($"Success! Material [{finalMatID}] successfully linked to existing supplier ({finalSuppID}).", "Transaction Successful", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            LoadExistingSuppliers(); RefreshSupplierGrid();
                            txtMatName.Clear(); txtMatPrice.Clear(); txtMatReorderLevel.Text = "20";
                            if (rbNewSupplier.Checked) { txtNewSuppName.Clear(); txtNewSuppContact.Clear(); }
                            if (rbExistingMaterial.Checked) LoadExistingMaterials();
                        }
                        catch (Exception innerEx) { trans.Rollback(); throw innerEx; }
                    }
                }
