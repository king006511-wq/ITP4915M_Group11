...
namespace ITP4915M_Group11
{
    ...
    public partial class OrderManagementForm : BaseForm
    {
        ...
        private ComboBox cboProducts, cboCustomers, cboRegion;
        ...
        private void InitializePremiumModernUI()
        {
            ...
            FlowLayoutPanel pnlSearch = new FlowLayoutPanel { Dock = DockStyle.Fill, Margin = new Padding(0), Padding = new Padding(0, 5, 0, 0), WrapContents = false };
            Label lblSearch = new Label { Text = "🔍 Search (Order / Customer):", Font = new Font("Segoe UI", 10F, FontStyle.Bold), ForeColor = Color.FromArgb(71, 85, 105), AutoSize = true, Margin = new Padding(0, 3, 10, 0) };
            txtSearch = new TextBox { Width = 250, Font = new Font("Segoe UI", 10.5F), BorderStyle = BorderStyle.FixedSingle };
            txtSearch.TextChanged += txtSearch_TextChanged;
            pnlSearch.Controls.Add(lblSearch); pnlSearch.Controls.Add(txtSearch);
            ...
            int startY = 50;
            txtOrderID = CreateStyledTextBox(pnlCard, ref startY, "Order ID (Auto):", true, 210); startY -= 65;
            ...
            pnlCard.Controls.Add(lblCust); pnlCard.Controls.Add(cboCustomers); startY += 65;

            txtStaffUIID = CreateStyledTextBox(pnlCard, ref startY, "Staff ID:", true, 210); txtStaffUIID.Text = currentStaffID; startY -= 65;
            ...
            pnlCard.Controls.Add(lblCbo); pnlCard.Controls.Add(cboProducts); startY += 65;
            ...
            btnRemoveItem.Click += BtnRemoveItem_Click; pnlCard.Controls.Add(btnRemoveItem); startY += 70;
            ...
            Label lblRegion = new Label { Text = "🌍 Region / City:", Location = new Point(20, startY), AutoSize = true, Font = new Font("Segoe UI", 10F, FontStyle.Bold), ForeColor = Color.FromArgb(71, 85, 105) };
            cboRegion = new ComboBox { Location = new Point(180, startY - 3), Width = 160, DropDownStyle = ComboBoxStyle.DropDownList, Font = new Font("Segoe UI", 10.5F) };
            cboRegion.Items.AddRange(new string[] { "Hong Kong", "Tokyo", "Singapore", "New York", "London" });
            cboRegion.SelectedIndex = 0;
            pnlCard.Controls.Add(lblRegion); pnlCard.Controls.Add(cboRegion); startY += 40;
            ...
            btnClear.Click += (s, e) => ClearFields(); pnlCard.Controls.Add(btnClear); startY += 55;
        ...
        private void DgvOrders_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            foreach (DataGridViewRow row in dgvOrders.Rows)
            {
                ...
                if (lowerStatus.Contains("cancel") || lowerStatus.Contains("reject") || lowerStatus.Contains("取消"))
                {
                    row.Cells["Status"].Style.ForeColor = Color.Red;
                    row.Cells["Status"].Style.Font = new Font(dgvOrders.Font, FontStyle.Bold);
                }
                else if (lowerStatus.Contains("complet") || lowerStatus.Contains("done") || lowerStatus.Contains("完成") || lowerStatus.Contains("自取"))
                {
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                string keyword = txtSearch.Text.Trim().Replace("'", "''").Replace("[", "[[]").Replace("]", "[]]").Replace("*", "[*]").Replace("%", "[%]");
                dt.DefaultView.RowFilter = string.IsNullOrEmpty(keyword) ? "" : "[Order ID] LIKE '%" + keyword + "%' OR [Customer ID] LIKE '%" + keyword + "%'";
            }
        }
        ...
        private void LoadCustomersToCombo()
        {
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                ...
                catch (Exception ex) { HandleSecureException(ex, "Load Customers List"); }
            }
        }
        ...
        private void LoadProductsToCombo()
        {
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                ...
                catch (Exception ex) { HandleSecureException(ex, "Load Products List"); }
            }
        }
        ...
        private void BtnAddItem_Click(object sender, EventArgs e)
        {
            if (cboProducts.SelectedItem == null)
            {
                MessageBox.Show("Select product.", "Validation Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }
            if (!int.TryParse(txtQty.Text.Trim(), out int qty) || qty <= 0)
            {
                MessageBox.Show("Invalid quantity value.", "Validation Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }
            if (qty > 9999)
            {
                MessageBox.Show("Quantity exceeds allowable systemic boundary limits per item (Max 9,999).", "Security Guardrail", MessageBoxButtons.OK, MessageBoxIcon.Hand); return;
            }
            ...
            foreach (DataRow row in cartTable.Rows)
            {
                if (row["ProductID"].ToString() == prod.ID)
                {
                    ...
                    if (totalQty > 99999)
                    {
                        MessageBox.Show("Accumulated quantity limits exceeded for this line item.", "Security Guardrail", MessageBoxButtons.OK, MessageBoxIcon.Hand); return;
                    }
                    row["Qty"] = totalQty;
        ...
        private void dgvOrders_SelectionChanged(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand("SELECT CustomerID,StaffID,Status,DeliveryDate FROM orders WHERE OrderID=@OID", conn))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                {
                                    if (item.ID == cID) { cboCustomers.SelectedItem = item; break; }
                                }
                                ...
                                {
                                    if (currentStatus.Contains($"[{city}]")) { cboRegion.Text = city; regionMatched = true; break; }
                                }
                                if (!regionMatched) cboRegion.SelectedIndex = 0;
                                try
                                {
                ...
                catch (Exception ex) { HandleSecureException(ex, "Select/Retrieve Sales Order Details"); }
            }
        }
                ...
                try
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand("SELECT OrderID FROM orders WHERE OrderID LIKE @Prefix ORDER BY OrderID DESC LIMIT 1", conn))
                    {
                        ...
                        if (res != null && res != DBNull.Value)
                        {
                            ...
                            {
                                txtOrderID.Text = prefix + (seq + 1).ToString("D4"); return;
                            }
                        }
                    }
        ...
        private void btnCreateOrder_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Customer/Cart cannot be empty.", "Validation Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }
            string orderID = txtOrderID.Text.Trim();
            string baseStatus = chkRequireDelivery.Checked ? "Awaiting Approval-D" : "Awaiting Approval-P";
            string orderStatus = $"{baseStatus} [{cboRegion.Text}]";
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                    ...
                    using (MySqlCommand checkCmd = new MySqlCommand("SELECT COUNT(*) FROM orders WHERE OrderID=@OID", conn))
                    {
                        ...
                        {
                            MessageBox.Show("Order already exists. Click Update if changes are needed.", "Data Conflict", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
                        }
                    }
                ...
                catch (Exception ex) { HandleSecureException(ex, "Execute Create Order Transaction"); }
            }
        }
            ...
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                    ...
                    using (MySqlCommand statusCmd = new MySqlCommand("SELECT Status FROM orders WHERE OrderID=@OID", conn))
                    {
                        ...
                        if (res == null)
                        {
                            MessageBox.Show("This is a NEW order ID. Click 'Create Order'.", "Invalid operation", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
                        }
                        currentStatus = res.ToString();
                    }
                    if (!currentStatus.StartsWith("Awaiting Approval") && !currentStatus.Contains("Rejected"))
                    {
                        MessageBox.Show("🔒 This order has already been Approved, Processed, or Completed!\n\nYou cannot modify a locked order.", "Order System Locked", MessageBoxButtons.OK, MessageBoxIcon.Stop); return;
                    }
                    string baseStatus = chkRequireDelivery.Checked ? "Awaiting Approval-D" : "Awaiting Approval-P";
                    string orderStatus = $"{baseStatus} [{cboRegion.Text}]";
                    using (MySqlTransaction trans = conn.BeginTransaction())
                ...
                catch (Exception ex) { HandleSecureException(ex, "Execute Update Order Transaction"); }
            }
        }

        // 🌟 核心修復：Pickup Done 而家會嚴格檢查系統庫存狀態，未批准嘅一律唔放行！
        private void btnCompletePickup_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                    conn.Open();
                    string currentStatus = "";
                    using (MySqlCommand cmd = new MySqlCommand("SELECT Status FROM orders WHERE OrderID=@OID", conn))
                    {
                    ...
                    // 1. 攔截「未批核」訂單
                    if (currentStatus.Contains("Awaiting Approval"))
                    {
                        MessageBox.Show("This order has NOT been approved yet!\n\nAll orders (including self-pickup) must be verified and approved by a Manager/Admin to deduct inventory before handover.", "Approval Required", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    // 2. 攔截「非自取」狀態訂單
                    if (!currentStatus.StartsWith("Ready for Pickup"))
                    {
                        MessageBox.Show($"Current status is '{currentStatus}'.\nOnly orders marked as 'Ready for Pickup' can be completed here.", "Invalid Action", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    DialogResult resConfirm = MessageBox.Show($"Is the customer currently picking up the items?\n\nAre you sure you want to mark order [{orderID}] as 'Pickup Completed'?", "Confirm Completion", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (resConfirm == DialogResult.Yes)
                    {
                        string regionSuffix = currentStatus.Contains("[") ? currentStatus.Substring(currentStatus.IndexOf("[")) : $"[{cboRegion.Text}]";
                        string newStatus = $"Pickup Completed {regionSuffix}";

                        using (MySqlCommand cmd = new MySqlCommand("UPDATE orders SET Status=@Status WHERE OrderID=@OID", conn))
                        {
        ...
        private void BtnCreateQuotation_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog { Filter = "HTML Web Document (*.html)|*.html", FileName = $"Order_{txtOrderID.Text}.html" })
            {
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    catch (Exception ex) { HandleSecureException(ex, "Generate Safe Export HTML Document"); }
                }
            }
        }
        ...
        private void RefreshOrdersGrid()
        {
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                ...
                catch (Exception ex) { HandleSecureException(ex, "Refresh Orders List Grid"); }
            }
        }
