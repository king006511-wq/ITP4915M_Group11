...
namespace ITP4915M_Group11
{
    ...
    public partial class ProcurementForm : Form
    {
        ...
        private TextBox txtPendingSearch;
        ...
        private void SetupCustomSleekUI()
        {
            ...
            custom_pnlLeftCard.Controls.Add(lblMat);
            custom_pnlLeftCard.Controls.Add(custom_txtMaterialID);
            custom_pnlLeftCard.Controls.Add(custom_txtMaterialName);
            startY += 60;
            ...
            custom_btnAddAllocation.FlatAppearance.BorderSize = 0;
            custom_btnRemoveAllocation.FlatAppearance.BorderSize = 0;
            custom_btnAddAllocation.Click += BtnAddAllocation_Click;
            custom_btnRemoveAllocation.Click += BtnRemoveAllocation_Click;
            custom_pnlLeftCard.Controls.Add(custom_btnAddAllocation);
            custom_pnlLeftCard.Controls.Add(custom_btnRemoveAllocation);
            startY += 50;
            ...
            custom_dgvPOCart.RowsDefaultCellStyle.Font = new Font("Segoe UI", 10F, FontStyle.Regular);
            ...
            // 🎯 Pending Request 嘅 Search Panel
            ...
            pnlPendingSearch.Controls.Add(lblSearchPending);
            pnlPendingSearch.Controls.Add(txtPendingSearch);
            ...
            custom_dgvPendingRC.EnableHeadersVisualStyles = false; custom_dgvPendingRC.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(37, 99, 235); custom_dgvPendingRC.ColumnHeadersDefaultCellStyle.ForeColor = Color.White; custom_dgvPendingRC.RowsDefaultCellStyle.Font = new Font("Segoe UI", 10F, FontStyle.Regular);
            ...
            custom_dgvDetails.EnableHeadersVisualStyles = false; custom_dgvDetails.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(14, 165, 233); custom_dgvDetails.ColumnHeadersDefaultCellStyle.ForeColor = Color.White; custom_dgvDetails.RowsDefaultCellStyle.Font = new Font("Segoe UI", 10F, FontStyle.Regular);
        ...
        private TextBox CreateCustomTextBox(Panel container, ref int topY, string labelText, bool readOnly, int width, int offsetX = 20)
        {
            ...
            container.Controls.Add(lbl); container.Controls.Add(txt); topY += 60; return txt;
        }
        ...
        private void LoadPendingRequests()
        {
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                    ...
                    {
                        DataTable dt = new DataTable(); adapter.Fill(dt); custom_dgvPendingRC.DataSource = dt;
                    }
                }
        ...
        private void LoadPOHistory()
        {
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                    ...
                    using (MySqlDataAdapter adapter = new MySqlDataAdapter(query, conn))
                    {
                        DataTable dt = new DataTable(); adapter.Fill(dt); dgvPOList.DataSource = dt;
                    }
                }
        ...
        private void TxtPendingSearch_TextChanged(object sender, EventArgs e)
        {
            if (custom_dgvPendingRC.DataSource is DataTable dt)
            {
                ...
                dt.DefaultView.RowFilter = string.IsNullOrWhiteSpace(keyword) ? "" : $"[Request ID] LIKE '%{keyword}%'";
            }
        }
        #endregion
        ...
        private void BtnAddAllocation_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select material, vendor and enter valid quantity/price.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }
            var sup = (POSupplierItem)custom_cmbSupplier.SelectedItem; string rcID = custom_txtRCID.Text.Trim(); string matID = custom_txtMaterialID.Text.Trim();
            decimal subtotal = qty * price; bool exists = false;
            ...
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                    ...
                    using (MySqlTransaction trans = conn.BeginTransaction())
                    {
                        try
                        {
                            var supplierGroups = poCartTable.AsEnumerable().GroupBy(r => r.Field<string>("Sup.ID").Trim().ToUpper()).ToList();
                            string prefix = "PO" + DateTime.Now.ToString("yyyyMMdd") + "-"; int nextSeq = 1;
                            using (MySqlCommand seqCmd = new MySqlCommand("SELECT PO_ID FROM purchase_order WHERE PO_ID LIKE @prefix ORDER BY PO_ID DESC LIMIT 1", conn, trans)) { seqCmd.Parameters.AddWithValue("@prefix", prefix + "%"); object result = seqCmd.ExecuteScalar(); if (result != null && result != DBNull.Value) { string seqStr = result.ToString().Replace(prefix, ""); if (int.TryParse(seqStr, out int seq)) nextSeq = seq + 1; } }
                            string generatedPOs = "";
                            foreach (var group in supplierGroups)
                            {
                                string supID = group.First().Field<string>("Sup.ID"); string poID = prefix + nextSeq.ToString("D3"); nextSeq++; string associatedRCID = group.First().Field<string>("Request ID");
                                ...
                                using (MySqlCommand cmd = new MySqlCommand(insertPO, conn, trans)) { cmd.Parameters.AddWithValue("@PO", poID); cmd.Parameters.AddWithValue("@Sup", supID); cmd.Parameters.AddWithValue("@RC", associatedRCID); cmd.Parameters.AddWithValue("@Staff", staffID); cmd.ExecuteNonQuery(); }
                                string insertLine = "INSERT INTO po_lineitem (PO_ID,MaterialID,Quantity,UnitPrice) VALUES (@PO,@Mat,@Qty,@Price)";
                                using (MySqlCommand cmd = new MySqlCommand(insertLine, conn, trans)) { foreach (var row in group) { cmd.Parameters.Clear(); cmd.Parameters.AddWithValue("@PO", poID); cmd.Parameters.AddWithValue("@Mat", row["Mat.ID"]); cmd.Parameters.AddWithValue("@Qty", row["Qty"]); cmd.Parameters.AddWithValue("@Price", row["Price"]); cmd.ExecuteNonQuery(); } }
                                string updateRC = "UPDATE reorder_card SET Status='Approved' WHERE ReOrderCardID=@RC AND MaterialID=@Mat";
                                using (MySqlCommand cmd = new MySqlCommand(updateRC, conn, trans)) { foreach (var row in group) { cmd.Parameters.Clear(); cmd.Parameters.AddWithValue("@RC", row["Request ID"]); cmd.Parameters.AddWithValue("@Mat", row["Mat.ID"]); cmd.ExecuteNonQuery(); } }
                                generatedPOs += poID + "\n";
                            }
                            trans.Commit();
        ...
        {
            string rcID = custom_txtRCID.Text.Trim(); if (string.IsNullOrWhiteSpace(rcID)) return;
            if (MessageBox.Show($"Reject entire request [{rcID}]?", "Confirm Rejection", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    try
                    {
                        conn.Open(); using (MySqlCommand cmd = new MySqlCommand("UPDATE reorder_card SET Status='Rejected' WHERE ReOrderCardID=@RC AND Status='Pending Approval'", conn)) { cmd.Parameters.AddWithValue("@RC", rcID); cmd.ExecuteNonQuery(); }
                        MessageBox.Show($"Request [{rcID}] rejected.", "Rejected", MessageBoxButtons.OK, MessageBoxIcon.Information); ClearCustomFields(); LoadPendingRequests();
                    }
        ...
        {
            custom_txtRCID.Clear(); custom_txtMaterialID.Clear(); custom_txtMaterialName.Clear(); custom_txtQty.Clear(); custom_txtPrice.Clear(); poCartTable.Clear(); UpdateGlobalTotal(); custom_cmbSupplier.DataSource = null; custom_dgvPendingRC.ClearSelection(); custom_dgvDetails.DataSource = null;
        }
        #endregion
    }
}